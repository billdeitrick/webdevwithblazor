...
public class BlogApiWebClient : IBlogApi
{
    ...
    public async Task<BlogPost?> SaveBlogPostAsync(BlogPost item)
    {
        ...
        try
        {
            ...
            var response = await httpclient.PostAsJsonAsync<BlogPost>("/api/BlogPosts", item);
    ...
    public async Task<Category?> SaveCategoryAsync(Category item)
    {
        ...
        {
            var httpclient = _factory.CreateClient("Api");
            var response = await httpclient.PostAsJsonAsync<Category>("/api/Categories", item);
            var json = await response.Content.ReadAsStringAsync();
    ...
    public async Task<Tag?> SaveTagAsync(Tag item)
    {
        ...
        {
            var httpclient = _factory.CreateClient("Api");
            var response = await httpclient.PostAsJsonAsync<Tag>("/api/Tags", item);
            var json = await response.Content.ReadAsStringAsync();
    ...
    {
        var httpclient = _factory.CreateClient("Api");
        return await httpclient.GetFromJsonAsync<List<Comment>>($"/api/Comments/{id}");
    }
    ...
    public async Task<Comment?> SaveCommentAsync(Comment item)
    {
        ...
        {
            var httpclient = _factory.CreateClient("Api");
            var response = await httpclient.PostAsJsonAsync<Comment>("/api/Comments", item);
            var json = await response.Content.ReadAsStringAsync();
